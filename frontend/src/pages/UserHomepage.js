...
const UserHomepage = () => {
  ...
  const [displayedTours, setDisplayedTours] = useState([]); // Fixed state for filtered/sorted tours
  const [loadingTours, setLoadingTours] = useState(false);
  const [filterOption, setFilterOption] = useState("Date"); // State for filter option
  ...
  useEffect(() => {
    const fetchFeaturedTours = async () => {
      try {
        ...
        // Filter to only show active safari packages
        const activeTours = response.data.filter(tour => tour.status === 'active');
        
        setFeaturedTours(activeTours);
        setDisplayedTours(activeTours); // Initially set displayed tours to active tours
  ...
  // Apply filter/sort whenever filterOption changes or when featuredTours updates
  useEffect(() => {
